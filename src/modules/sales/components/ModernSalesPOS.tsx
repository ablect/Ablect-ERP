...
export default function ModernSalesPOS() {
  ...
  const [sales, setLocalSales] = useState<
    Awaited<ReturnType<typeof saleService.getAll>>
  >([]);
  ...
  const discountAmount =
    subtotal * ((active?.discountPercent ?? 0) / 100);

  const taxable = Math.max(0, subtotal - discountAmount);

  const taxAmount =
    taxable * ((active?.taxPercent ?? 0) / 100);

  const grandTotal = Math.max(0, taxable + taxAmount);

  const amountPaid =
    active?.payments.reduce(
    ...
    updateActive((transaction) => ({
      ...
      payments: [
        ...transaction.payments,
        {
          id: crypto.randomUUID(),
          method: "Cash",
          amount: Math.max(0, grandTotal - amountPaid),
          reference: "",
        },
      ],
    }));
  ...
  async function completeCurrentSale() {
    ...
    if (!active.customerId) {
      setMessage(
        "Select a customer before completing the sale. Use the customer field for a walk-in record if configured.",
      );
      return;
    }
    ...
    for (const line of active.items) {
      ...
      if (!product) {
        setMessage(
          "One of the selected products no longer exists in inventory.",
        );
        return;
      }
      ...
      if (line.quantity > product.quantity) {
        setMessage(
          `${product.itemName} only has ${product.quantity} unit(s) available.`,
        );
        return;
      }
    }
    ...
    try {
      ...
      const saleItems = active.items.map((line) =>
        createSaleItem(
          saleId,
          line.productId,
          line.quantity,
      ...
      setMessage(
        `Sale ${active.invoiceNumber} completed successfully.`,
      );
    } catch (error) {
      setMessage(
    ...
    <div className="min-h-full space-y-5 pb-10">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Fast checkout, live stock control and auditable
            transactions.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
          >
            Sales history
          </button>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.55fr)_minmax(380px,0.85fr)]">
            ...
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex items-center gap-2 overflow-x-auto pb-1">
                ...
                <button
                  ...
                  className="flex shrink-0 items-center gap-1 rounded-xl border border-dashed border-slate-300 px-3 py-2 text-xs font-semibold text-slate-500 hover:border-slate-500 hover:text-slate-900"
                >
                  <Plus size={14} />
                  New order
                </button>
          ...
          <div className="p-4">
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-2xl border border-slate-200 bg-slate-50 py-3 pl-10 pr-12 text-sm outline-none transition focus:border-slate-400 focus:bg-white focus:ring-4 focus:ring-slate-100"
                autoFocus
              />
            ...
            <div className="mt-4 grid max-h-[475px] grid-cols-2 gap-3 overflow-y-auto pr-1 sm:grid-cols-3 lg:grid-cols-4">
              {filteredProducts.map((product) => (
                <button
                  ...
                  className="group rounded-2xl border border-slate-200 bg-white p-3 text-left shadow-sm transition hover:-translate-y-1 hover:border-slate-300 hover:shadow-lg disabled:cursor-not-allowed disabled:opacity-50"
                  disabled={product.quantity <= 0}
                >
                  <div className="flex h-24 items-center justify-center rounded-xl bg-gradient-to-br from-slate-100 via-white to-slate-200 text-slate-300 transition group-hover:from-slate-200 group-hover:text-slate-500">
              ...
              {filteredProducts.length === 0 && (
                <div className="col-span-full rounded-2xl border border-dashed border-slate-300 p-10 text-center text-sm text-slate-500">
                  No products match this search. Add products in
                  Inventory, then refresh stock.
                </div>
              )}
            </div>
          </div>
        ...
        <section className="flex min-h-[620px] flex-col rounded-3xl border border-slate-200/80 bg-slate-950 text-white shadow-xl shadow-slate-200/40">
            ...
            <div className="space-y-2">
              ...
              {!active?.items.length && (
                <div className="rounded-2xl border border-dashed border-white/15 p-10 text-center">
                  <ShoppingCart className="mx-auto text-slate-500" />
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    Select a product to start this transaction.
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="border-t border-white/10 p-4">
            ...
            <button
              disabled={
                processing || !active?.items.length
              }
              onClick={() => {
                playUiClick();
                setShowPayment(true);
              }}
              className="mt-4 flex w-full items-center justify-center gap-2 rounded-2xl bg-white px-4 py-3.5 text-sm font-bold text-slate-950 transition hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-40"
            >
              <WalletCards size={18} />
              Continue to payment
            </button>
      ...
      <section className="grid gap-5 lg:grid-cols-[1.35fr_0.65fr]">
        <div className="rounded-3xl border border-slate-200/80 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500">
                Completed sales over the last seven days.
              </p>
            </div>
          ...
          <div className="mt-5 flex h-40 items-end gap-3">
            {lastSeven.map((item) => (
              ...
              >
                <div
                  className="w-full rounded-t-xl bg-gradient-to-t from-slate-900 to-slate-500 transition hover:from-slate-700"
                  style={{
                    height: `${Math.max(
                      8,
                      (item.value / maxChart) * 100,
                    )}%`,
                  }}
        ...
        <div className="rounded-3xl border border-slate-200/80 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 grid grid-cols-2 gap-2 text-xs font-semibold">
            <button
              onClick={addTransaction}
              className="rounded-xl bg-slate-100 px-3 py-3 text-slate-700 hover:bg-slate-200"
            >
              New order
            </button>
            ...
            <button
              onClick={() =>
                setMessage(
                  "Scanner ready. Focus the product search and scan a SKU/barcode.",
                )
              }
              className="rounded-xl bg-slate-100 px-3 py-3 text-slate-700 hover:bg-slate-200"
            >
          ...
          {message && (
            <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs font-medium text-slate-600">
              {message}
            </div>
          )}
        </div>
      ...
      {showDiscount && active && (
        <div
          ...
          onMouseDown={() => setShowDiscount(false)}
        >
          <div
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-slate-500">
                  Apply the discount once at order level.
                </p>
              </div>
        ...
        >
          ...
          >
            ...
            <div className="mt-5 space-y-3">
              ...
              <button
                onClick={addPayment}
                className="rounded-xl border border-dashed border-slate-300 px-4 py-2 text-sm font-semibold text-slate-600 hover:border-slate-500"
              >
                + Split payment
              </button>
            </div>
            ...
            {balanceDue > 0 && (
              <div className="mt-4 rounded-xl bg-amber-50 p-4 text-sm font-semibold text-amber-800">
                Outstanding: {money(balanceDue)}. Use
                Credit for an approved credit sale.
              </div>
            )}

            <button
              disabled={processing}
              onClick={completeCurrentSale}
              className="mt-6 flex w-full items-center justify-center gap-2 rounded-2xl bg-slate-900 px-5 py-4 text-sm font-bold text-white transition hover:bg-slate-800 disabled:opacity-50"
            >
              <CreditCard size={18} />
      ...
      {showHistory && (
        ...
        >
          <div
            className="max-h-[88vh] w-full max-w-5xl overflow-y-auto rounded-3xl bg-white p-6 shadow-2xl"
            onMouseDown={(event) =>
              event.stopPropagation()
            }
          >
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-slate-500">
                  Search invoices without leaving the POS
                  workspace.
                </p>
              </div>
            ...
            <div className="relative mt-5">
              ...
              <input
                ...
                className="w-full rounded-2xl border bg-slate-50 py-3 pl-10 pr-4 text-sm outline-none focus:bg-white"
