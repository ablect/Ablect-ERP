...
interface AdminState {
  users: SystemUser[];
  roles: Record<AdminRole, PermissionMatrix>;
  auditLogs: AuditLog[];
  selectedRole: AdminRole;
  setSelectedRole: (role: AdminRole) => void;
  togglePermission: (role: AdminRole, module: AppModule, action: PermissionAction) => void;
  setUserRole: (userId: string, role: AdminRole) => void;
  setUserStatus: (userId: string, status: SystemUser["status"]) => void;
