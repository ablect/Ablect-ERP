...
function normalize(product: InventoryItem | Product): InventoryItem {
  if ("itemName" in product) return product;
...
export default function ProductTable({
        ...
        <div className="flex min-w-[240px] items-center gap-3">
          <div className="h-10 w-10 shrink-0 overflow-hidden rounded-xl bg-indigo-50">
            {row.original.imageUrl ? (
              ...
              <div className="flex h-full items-center justify-center text-indigo-300"><PackageOpen size={18} /></div>
            )}
          </div>
          <div className="min-w-0">
            <p className="truncate font-bold text-slate-900">{row.original.itemName}</p>
            <p className="truncate text-xs text-slate-400">{row.original.brand || "No brand"}</p>
          </div>
        </div>
      ),
    },
  ...
  if (!products.length) {
    return <div className="rounded-3xl border border-dashed border-slate-300 bg-white p-16 text-center"><PackageOpen className="mx-auto text-slate-300" size={44} /><h2 className="mt-4 text-lg font-black">No products found</h2></div>;
  }

  return (
    <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
      <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
        <p className="text-sm font-bold text-slate-800">Catalog data grid</p>
        <span className="text-xs font-semibold text-slate-400">{products.length.toLocaleString()} rows</span>
      </div>
        ...
        <table className="w-full min-w-[1000px] text-sm">
              ...
              <tr key={group.id}>
                {group.headers.map((header) => (
                  <th key={header.id} className="px-4 py-3">
                    {header.isPlaceholder ? null : header.id === "select" ? flexRender(header.column.columnDef.header, header.getContext()) : (
                      <button disabled={!header.column.getCanSort()} onClick={header.column.getToggleSortingHandler()} className="inline-flex items-center gap-1">
                        {flexRender(header.column.columnDef.header, header.getContext())}
                        {header.column.getIsSorted() === "asc" ? <ArrowUp size={12} /> : header.column.getIsSorted() === "desc" ? <ArrowDown size={12} /> : header.column.getCanSort() ? <ChevronUp size={12} className="opacity-30" /> : null}
                      </button>
                    )}
                  </th>
                ))}
          ...
          <tbody>
            {table.getRowModel().rows.map((row) => (
              <tr key={row.id} onClick={() => onOpen?.(row.original)} className={`border-t border-slate-100 transition ${onOpen ? "cursor-pointer hover:bg-indigo-50/40" : ""}`}>
                {row.getVisibleCells().map((cell) => <td key={cell.id} className="px-4 py-4">{flexRender(cell.column.columnDef.cell, cell.getContext())}</td>)}
              </tr>
            ))}
          </tbody>
        </table>
