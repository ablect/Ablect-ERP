      ...
      return withTransaction(pool, async (connection) => {
        ...
        for (const item of items) {
          ...
          const [rows] = await connection.query(
            `SELECT id, name, selling_price, quantity FROM products WHERE id = ? FOR UPDATE`,
            [item.productId],
          );
        ...
        // The database auto-increment primary key is the authoritative sequence.
        // A temporary unique number lets us insert the row first, obtain its
        // atomic ID, and then replace it with the permanent invoice number.
        const temporaryNumber = `TMP-${crypto.randomUUID()}`;
        const [result] = await connection.query(
          `INSERT INTO sales_orders
             (sale_number, customer_id, warehouse_id, status, payment_status,
              payment_method, subtotal, total, paid_amount, created_by)
           VALUES (?, ?, ?, 'COMPLETED', ?, ?, ?, ?, ?, ?)`,
        ...
        for (const item of normalized) {
          await connection.query(
            `INSERT INTO sales_order_items
             ...
             VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [result.insertId, item.productId, item.quantity, item.unitPrice, item.discount, item.tax, item.lineTotal],
          );
          ...
          await connection.query(
            `INSERT INTO stock_movements
               (product_id, movement_type, quantity, source_warehouse_id,
                reference_type, reference_id, performed_by)
             VALUES (?, 'OUT', ?, ?, 'SALE', ?, ?)`,
            [item.productId, item.quantity, warehouseId ?? null, invoiceNumber, userId ?? null],
          );
        }

        if (customerId) {
          await connection.query(`UPDATE customers SET loyalty_points = loyalty_points + FLOOR(? / 100) WHERE id = ?`, [total, customerId]);
        }

        await connection.query(
          `INSERT INTO audit_logs (user_id, action, entity_type, entity_id, details)
           VALUES (?, 'CREATE', 'SALE', ?, JSON_OBJECT('invoiceNumber', ?, 'total', ?, 'items', ?))`,
          [userId ?? null, String(result.insertId), invoiceNumber, total, normalized.length],
        );
