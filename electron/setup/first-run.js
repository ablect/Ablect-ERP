...
export async function completeFirstRun(pool, userDataPath, payload) {
  ...
  try {
    ...
    if (admin) {
      await connection.query(`UPDATE users SET username=?,email=?,password_hash=?,full_name=?,role='Administrator',is_active=TRUE WHERE id=?`, [email, email, passwordHash, fullName, admin.id]);
    } else {
      const [[role]] = await connection.query(`SELECT id FROM roles WHERE name='Administrator' LIMIT 1`);
      await connection.query(`INSERT INTO users (username,email,password_hash,full_name,role,role_id,is_active) VALUES (?,?,?,?, 'Administrator',?,TRUE)`, [email, email, passwordHash, fullName, role?.id ?? null]);
    }
